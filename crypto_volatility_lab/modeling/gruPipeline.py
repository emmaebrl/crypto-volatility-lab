...
from tensorflow.keras.layers import GRU, Dense, Reshape  # type: ignore
...
class GRUPipeline:
    ...
    def create_gru_model(self, input_shape, forecast_horizon, num_features):
        model = Sequential(
            [
                GRU(self.gru_units, activation="relu", input_shape=input_shape),
    ...
    def process_window(self, data_scaled, X, y, indices):
        ...
        model = self.create_gru_model(
            X_train.shape[1:], self.forecast_horizon, data_scaled.shape[1]
        )
        model.fit(
            X_train, y_train, epochs=self.epochs, batch_size=self.batch_size, verbose=0
        )
    ...
    def run(self, data):
        ...
        results = Parallel(n_jobs=self.n_jobs)(
            delayed(self.process_window)(data_scaled, X, y, indices)
            for indices in tqdm(indices_list, desc="Training progress")
        )
